//=============================================================================
// Router + one-core_group MNIST classifier testbench
//
// Labels are not used during unsupervised training. They are used only after
// training for output-neuron label assignment and final evaluation.
//=============================================================================

`timescale 1ns / 1ps

module tb_custom_rtl_mnist_router_coregroup_classifier;
    ...
    wire [31:0] router_learning_weight_writes;
    ...
    custom_rtl_mnist_router_coregroup_classifier_top dut (
        .clk(clk),
        .rst_n(rst_n),
        .enable(enable),
        ...
        .router_learning_weight_writes(router_learning_weight_writes),
        .direct_learning_write_count(direct_learning_write_count)
    );

    task automatic check;
        input [8*112-1:0] desc;
        input condition;
    begin
        if (condition) begin
    ...
    begin
        ...
        timeout = 0;
        while (!init_done && timeout < 20000) begin
            @(posedge clk);
            timeout = timeout + 1;
        end
        if (!init_done) begin
            $display("[ERROR] router coregroup classifier init timed out");
            $finish(2);
        end
    ...
    begin
        ...
        timeout = 0;
        while (!image_done && timeout < 350000) begin
            @(posedge clk);
            timeout = timeout + 1;
        end
        if (!image_done) begin
    ...
    begin
        ...
        for (img = 0; img < train_image_count; img = img + 1) begin
            ...
            if ((img % 25) == 0)
                $display("router train image=%0d winner=%0d valid=%0d out_spikes=%0d latency=%0d",
    ...
    begin
        ...
        for (out_idx = 0; out_idx < 10; out_idx = out_idx + 1) begin
            ...
            $display("router assigned output %0d -> digit %0d count=%0d wins=%0d",
    ...
    begin
        ...
        for (img = 0; img < test_image_count; img = img + 1) begin
            ...
            $display("router test image=%0d label=%0d winner=%0d valid=%0d pred=%0d out_spikes=%0d latency=%0d",
    ...
    initial begin
        ...
        $display("=========================================================");
        $display("  Router + one-core_group MNIST classifier TB subset=%0s", subset_name);
        $display("=========================================================");
        ...
        $display("router confusion matrix:");
        ...
        check("event_router_ng routed external input spikes", router_routed_spike_count > 0);
        check("natural core_group output spikes observed through router handshake", output_spike_sum > 0);
        check("router learn_spike observation path saw core_group spikes", router_observed_spike_count > 0);
        check("STDP wrote learned weights through event_router_ng.learn_weight_*", router_learning_weight_writes > 0);
        check("direct core_group learning writes avoided by this top", direct_learning_write_count == 0);
        check("assignment and test images processed", test_image_count > 0 && train_image_count > 0);

        $display("MNIST_ROUTER_COREGROUP_RTL_SUMMARY subset=%0s train_images=%0d test_images=%0d accuracy_permille=%0d avg_latency_cycles=%0d avg_output_spikes_x1000=%0d avg_input_spikes_x1000=%0d throughput_images_per_sec=%0d weight_min=%0d weight_max=%0d weight_sum=%0d total_updates=%0d router_weight_writes=%0d router_routed_spikes=%0d router_observed_spikes=%0d direct_learning_writes=%0d sim_cycles=%0d",
                 ...
                 total_weight_updates, router_learning_weight_writes,
                 router_routed_spike_count, router_observed_spike_count,
                 direct_learning_write_count, sim_cycle);

        if (fail_count != 0) begin
            $display("*** MNIST ROUTER COREGROUP CLASSIFIER RTL TEST FAILED ***");
            $finish(1);
        end

        $display("*** MNIST ROUTER COREGROUP CLASSIFIER RTL TEST PASSED ***");
        $finish(0);
    end

    initial begin
        #250000000;
        $display("[ERROR] router coregroup classifier TB timed out");
        $finish(2);
    end

#=============================================================================
# Run RTL-only MNIST classifier simulations with Vivado xsim.
#
# Usage:
...
set work_dir [file join $repo_root "hardware" "sim_work_rtl_mnist_classifier"]
...
set files [list \
    ...
    [file join $rtl_dir "top" "custom_rtl_mnist_classifier_top.v"] \
    [file join $tb_dir "tb_custom_rtl_mnist_classifier.v"] \
]

proc run_one {subset files inc_dir data_dir work_dir} {
    set name "tb_custom_rtl_mnist_classifier_${subset}"
    puts ""
    puts "============================================================"
    puts "Running $name"
    puts "============================================================"
    ...
    if {[catch {exec xelab -nolog -debug typical tb_custom_rtl_mnist_classifier -s ${name}_sim > $elab_log 2>@1} err]} {
    ...
    foreach line [split $data "\n"] {
        if {[regexp {(\[PASS\]|\[FAIL\]|ERROR|SUMMARY|MNIST_CLASSIFIER_RTL_SUMMARY|assigned output|Accuracy|processed)} $line]} {
            puts $line
        }
...
puts ""
puts "Custom RTL MNIST classifier xsim run complete."

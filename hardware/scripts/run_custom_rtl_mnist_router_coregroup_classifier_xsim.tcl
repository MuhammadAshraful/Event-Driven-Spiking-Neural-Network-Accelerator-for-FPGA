#=============================================================================
# Run router + one-core_group MNIST classifier simulations with Vivado xsim.
#
# Usage:
...
set work_dir [file join $repo_root "hardware" "sim_work_rtl_mnist_router_coregroup_classifier"]
...
set files [list \
    ...
    [file join $rtl_dir "top" "custom_rtl_mnist_router_coregroup_classifier_top.v"] \
    [file join $tb_dir "tb_custom_rtl_mnist_router_coregroup_classifier.v"] \
]

proc run_one {subset files inc_dir data_dir work_dir} {
    set name "tb_custom_rtl_mnist_router_coregroup_classifier_${subset}"
    puts ""
    puts "============================================================"
    puts "Running $name"
    puts "============================================================"
    ...
    if {[catch {exec xelab -nolog -debug typical tb_custom_rtl_mnist_router_coregroup_classifier -s ${name}_sim > $elab_log 2>@1} err]} {
    ...
    foreach line [split $data "\n"] {
        if {[regexp {(\[PASS\]|\[FAIL\]|ERROR|MNIST_ROUTER_COREGROUP_RTL_SUMMARY|router assigned output|ROUTER COREGROUP CLASSIFIER)} $line]} {
            puts $line
        }
...
puts ""
puts "Custom RTL MNIST router coregroup classifier xsim run complete."
